...
The **model** interprets alerts and user messages, selects available tools, and synthesizes conclusions from returned data. It does not query backends directly. The model requests a tool call; the runtime dispatches it, and the tool contract validates its inputs.
